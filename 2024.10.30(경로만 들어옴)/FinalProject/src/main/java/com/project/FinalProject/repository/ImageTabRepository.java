...
public interface ImageTabRepository extends JpaRepository<ImageTab, Long> {
 ...
 // studyKey, seriesKey, curSeqNum 조건으로 첫 번째 이미지 찾기
    @Query("SELECT i FROM ImageTab i WHERE i.studyKey = :studyKey AND i.seriesKey = :seriesKey AND i.curSeqNum = 1")
    Optional<ImageTab> findFirstImageByStudyKeyAndSeriesKeyAndCurSeqNum(
